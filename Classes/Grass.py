...
class Grass(object):
    def __init__(self,width,height):
        ...
        for i in range(0,self.resolution):
            self.box_centers.append([((i+.5)*self.box_width,(j+.5)*self.box_height+self.whitespace) for j in range(0, self.resolution)])
            self.box_corners.append([(i*self.box_width,j*self.box_height) for j in range(0, self.resolution)])
    ...
    def draw(self,win):
        for i in range(0,self.resolution):
            for j in range( 0, self.resolution):
                pygame.draw.rect(win,self.color[int(self.grass[i][j])],(i*self.box_width,j*self.box_height+self.whitespace,self.box_width,self.box_height))
