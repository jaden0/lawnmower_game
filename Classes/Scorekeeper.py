...
class Scorekeeper(object):
    ...
    def calculate(self, grass):
        ...
        self.score = np.count_nonzero(cut_grass) / (np.count_nonzero(uncut_grass) + np.count_nonzero(cut_grass)) * 100
    ...
    def draw_score(self, win):
        ...
        win.blit(text, (win.get_width() - text.get_width() * 1.2, -text.get_height()/2 + 25))

    def draw_final_score(self, win):
        font = pygame.font.SysFont("comicsa nsms", 30)
        text = font.render("Total grass cut: %.1f%%" % self.score, False, self.score_color)
        win.blit(text, (win.get_width()/2 - text.get_width()/2 , win.get_height()/2 - text.get_height()/2 + 50))

    def draw_timer(self, win):
        font = pygame.font.SysFont("comicsa nsms", 30)
        color = (self.score_color, (255,0,0))[self.end_time - time.time() < 10]
        text = font.render("Remaining Time: %.1f seconds" % (self.end_time - time.time()), False, color)
